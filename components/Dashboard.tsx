...
import type { Currency, ExpenseSource, Todo, GoalDetails } from '../types';
...
const { incomes: mockIncome, expenses: mockExpenses, todos: mockTodos, subscriptions: mockSubscriptions } = mockData;
...
const Dashboard: React.FC<DashboardProps> = ({ currency, onCategorySelect }) => {
  ...
  const todos: Todo[] = mockTodos as any;
    ...
    {showSubscriptions && (
        <ul className="space-y-4">
            {mockSubscriptions.map(sub => (
                <li key={sub.service} className="grid grid-cols-2 sm:grid-cols-4 gap-4 items-center bg-gray-800 p-4 rounded-lg">
